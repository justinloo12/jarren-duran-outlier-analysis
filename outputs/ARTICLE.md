...
The Red Sox are 53-50, and **Boston holds a wild card, 1.0 games clear of the cut line**. The record is the least informative number on the page. Boston has outscored opponents by **+41** runs this season, the run profile of a .545 team, and in 10,000 simulations of the rest of the season they make the playoffs **78%** of the time. This report works through what that means for the deadline: the buy/sell call, a position by position audit, who on the roster is outperforming their track record, the trades that fit, and the one already made.
...
- **And the awkward one: the reported sell candidate is their best hitter.** Willson Contreras carries a 146 wRC+ at first base. Moving him while +1.0 games out of a playoff spot would be met with outrage, and the outrage would be right.
...
The rotation gets the same screen, green arrows only, since underperformers are not regression risks: Sonny Gray is 0.80 runs under his career mark at age 36; Ranger Suarez is 0.23 runs under his career mark at age 30. Jake Bennett and Payton Tolle are rookies with no baseline, the same cuts-both-ways caveat as Cheng. Two starters outrunning their careers at once is one more quiet argument for adding innings before August 3.
