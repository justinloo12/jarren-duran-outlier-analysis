...
*Generated 2026-07-26 · data via pybaseball (Statcast), FanGraphs API, MLB Stats API*
...
- **2026's collapse overstates his decline: it is partly *unlucky*** (wOBA 0.260 sits *below* xwOBA 0.292; BABIP 0.233 vs xBABIP 0.313, -80 points — and for a speedster who normally *beats* his xstats, running below them is doubly anomalous). There is real erosion in 2026 — chase, whiff and hard-hit rate all worsened significantly — but no injury has been reported, his speed/defense remain plus, and the batting line is worse than the erosion warrants.
...
| 2026 * | 29 | 410 | 58 | 0.260 | 0.292 | 0.233 | 0.313 | 10.7% | 39.6% | 34.4% | 31.8% |
...
- 2026's gap sits **-35 to -45 points below his own baseline** — for a player whose legs normally add value over xstats, running negative is a far larger anomaly than the raw number shows.
...
| 2024 | +8.3 | +7.6 | +9.4 | 8.0 | 6.8 |
| 2025 | +7.2 | -2.0 | +3.7 | 7.2 | 3.9 |
| 2026 * | +2.9 | +1.7 | +6.0 | 6.7 | -0.1 |
...
- Actual drop 2024→2026 was **72** points — **65** beyond what aging explains.
...
Rest-of-season (235 PA at his current pace) simulated from a true-talent prior built on 2023–25 xwOBA (PA × 3/4/5 recency weights) plus his +3-pt speed premium — the healthy prior lands at **0.333 wOBA ≈ 109 wRC+**, independently confirming the ~110 plateau. Because §5 shows *real* 2026 process erosion, a second scenario blends 2026's degraded process (xwOBA 0.292) in at 40%. Full model + valuation math in `outputs/rebound_probability.md`; distribution in `figures/09_rebound_probability.png`.

| Quantity | Healthy prior | Erosion-blended |
|----------|--------------:|----------------:|
| P(rest-of-season wRC+ ≥ 100) | **65%** | **47%** |
| P(rest-of-season wRC+ ≥ 110) | 47% | 30% |
| P(full-season 2026 ≥ 90 wRC+) | 6% | 2% |
| Median rest-of-season wRC+ | 108 | 98 |
| Expected offseason trade value | $21M | $18M |

> **Read:** if the 2023–25 player is intact, a league-average-or-better second half is a 65% bet; if the process erosion is ~40% real it is a coin flip (47%) — that 18-point gap is what the eroded chase/whiff/hard-hit rates cost. Either way the full-season line likely stays under 90 wRC+ (the first-half hole is too deep), and either way waiting beats selling at today's ~$10M nadir — so the sim supports hold-through-2026, while lowering confidence in the rebound narrative itself.
...
- Duran's −32-pt gap is more negative than 89% of all qualified first halves, but *within* the unlucky cohort he is a typical member (48th percentile), not an outlier.
...
- **Quantified (§8):** the rebound Monte Carlo puts a league-average-or-better rest of season at **65%** (healthy prior) vs **47%** (erosion-blended), and expected offseason trade value at $21M vs $18M — both above the ~$10M a July sale fetches. Hold-through-2026 survives the erosion stress test.
