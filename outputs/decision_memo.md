...
*Generated 2026-07-19 · data via pybaseball (Statcast), FanGraphs API, MLB Stats API*
...
- **2026's collapse overstates his decline: it is partly *unlucky*** (wOBA 0.263 sits *below* xwOBA 0.293; BABIP 0.238 vs xBABIP 0.309, -71 points — and for a speedster who normally *beats* his xstats, running below them is doubly anomalous). There is real erosion in 2026 — chase, whiff and hard-hit rate all worsened significantly — but no injury has been reported, his speed/defense remain plus, and the batting line is worse than the erosion warrants.
...
| 2026 * | 29 | 387 | 60 | 0.263 | 0.293 | 0.238 | 0.309 | 11.2% | 38.4% | 34.8% | 32.4% |
...
- 2024's gap was **+5 to +14 points above his own baseline** — under any version, a modest luck component.
- 2026's gap sits **-34 to -43 points below his own baseline** — for a player whose legs normally add value over xstats, running negative is a far larger anomaly than the raw number shows.
...
- Actual drop 2024→2026 was **71** points — **63** beyond what aging explains.
...
Rest-of-season (253 PA at his current pace) simulated from a true-talent prior built on 2023–25 xwOBA (PA × 3/4/5 recency weights) plus his +3-pt speed premium — the healthy prior lands at **0.334 wOBA ≈ 109 wRC+**, independently confirming the ~110 plateau. Because §5 shows *real* 2026 process erosion, a second scenario blends 2026's degraded process (xwOBA 0.293) in at 40%. Full model + valuation math in `outputs/rebound_probability.md`; distribution in `figures/09_rebound_probability.png`.

| Quantity | Healthy prior | Erosion-blended |
|----------|--------------:|----------------:|
| P(rest-of-season wRC+ ≥ 100) | **66%** | **48%** |
| P(rest-of-season wRC+ ≥ 110) | 48% | 31% |
| P(full-season 2026 ≥ 90 wRC+) | 13% | 5% |
| Median rest-of-season wRC+ | 109 | 99 |
| Expected offseason trade value | $22M | $18M |

> **Read:** if the 2023–25 player is intact, a league-average-or-better second half is a 66% bet; if the process erosion is ~40% real it is a coin flip (48%) — that 18-point gap is what the eroded chase/whiff/hard-hit rates cost. Either way the full-season line likely stays under 90 wRC+ (the first-half hole is too deep), and either way waiting beats selling at today's ~$10M nadir — so the sim supports hold-through-2026, while lowering confidence in the rebound narrative itself.
...
> **Verdict: approach, not physical — and this is the tiebreaker the whole erosion question needed.** His bat speed is *up* +1.0 mph vs his 2024–25 average (fast-swing rate 34% → 47%), while his swing got longer and steeper and every *decision* metric worsened — whiffs up even against premium velocity he demonstrably still catches up to, chase up mostly on fastballs and offspeed, breaking-ball chase actually stable. That is a hitter **selling out**, not a hitter slowing down. Approach is fixable (his own 2022→23 chase overhaul proves it in-house); a dying bat is not. This tilts the §8 scenarios toward the healthy prior — a 60/40 weighting puts the blended P(rest-of-season wRC+ ≥ 100) at **59%**.
...
- Duran's −30-pt gap is more negative than 88% of all qualified first halves, but *within* the unlucky cohort he is a typical member (51th percentile), not an outlier.
...
- **Quantified (§8):** the rebound Monte Carlo puts a league-average-or-better rest of season at **66%** (healthy prior) vs **48%** (erosion-blended), and expected offseason trade value at $22M vs $18M — both above the ~$10M a July sale fetches. Hold-through-2026 survives the erosion stress test.
